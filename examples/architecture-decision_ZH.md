# 示例：微服务 vs 单体架构决策

> **核心 AHP 概念**：Leader Agent 优先级约束 —— 用户声明的优先级（"开发效率优先"）作为约束条件提升了某一准则的权重，展示了 AHP 如何将利益相关者的价值判断纳入数学框架。

[English](architecture-decision.md)

> 场景：电商平台日订单量从 1K 增长到 50K，现有 Django 单体出现性能瓶颈，团队考虑架构升级。

## Step 1: 决策框架构建

**决策目标**：选择最适合业务增长阶段的架构方案

**评估层次**：

```
选择最佳架构方案
├── 可扩展性
│   ├── 水平扩展能力
│   └── 独立部署能力
├── 开发效率
│   ├── 开发复杂度
│   ├── 团队协作效率
│   └── 上线速度
├── 可靠性
│   ├── 故障隔离
│   └── 数据一致性
└── 运维成本
    ├── 基础设施成本
    ├── 监控复杂度
    └── 运维人力需求
```

**备选方案**：
- **A. 全面微服务**：按领域拆分为独立服务
- **B. 模块化单体**：保持单体但严格模块化，关键模块可独立部署
- **C. 渐进式拆分**：先模块化，再逐步将瓶颈模块拆为独立服务

**用户优先级约束**：开发效率优先（创业阶段，需快速迭代）

## Step 2: 多视角评估

### 视角 1：技术架构师

| 对比 | 可扩展性 | 开发效率 | 可靠性 | 运维成本 |
|----|---------|---------|-------|---------|
| 可扩展性 | 1 | 1/2 | 2 | 3 |
| 开发效率 | 2 | 1 | 2 | 3 |
| 可靠性 | 1/2 | 1/2 | 1 | 2 |
| 运维成本 | 1/3 | 1/3 | 1/2 | 1 |

### 视角 2：运维工程师

| 对比 | 可扩展性 | 开发效率 | 可靠性 | 运维成本 |
|----|---------|---------|-------|---------|
| 可扩展性 | 1 | 1/2 | 1 | 1/2 |
| 开发效率 | 2 | 1 | 2 | 1 |
| 可靠性 | 1 | 1/2 | 1 | 1 |
| 运维成本 | 2 | 1 | 1 | 1 |

### 视角 3：CTO

| 对比 | 可扩展性 | 开发效率 | 可靠性 | 运维成本 |
|----|---------|---------|-------|---------|
| 可扩展性 | 1 | 1/3 | 2 | 2 |
| 开发效率 | 3 | 1 | 3 | 4 |
| 可靠性 | 1/2 | 1/3 | 1 | 1 |
| 运维成本 | 1/2 | 1/4 | 1 | 1 |

## Step 3: 共识聚合

### 几何平均矩阵

| 对比 | 可扩展性 | 开发效率 | 可靠性 | 运维成本 |
|----|---------|---------|-------|---------|
| 可扩展性 | 1 | 0.44 | 1.59 | 1.44 |
| 开发效率 | 2.29 | 1 | 2.29 | 2.29 |
| 可靠性 | 0.63 | 0.44 | 1 | 1.26 |
| 运维成本 | 0.69 | 0.44 | 0.79 | 1 |

### 权重计算

| 准则 | 行几何均值 | 归一化权重 |
|-----|-----------|----------|
| 可扩展性 | 1.04 | 0.28 |
| 开发效率 | 1.87 | 0.38 ×（已应用约束） |
| 可靠性 | 0.74 | 0.20 |
| 运维成本 | 0.65 | 0.14 |

**权重分布**（已应用"开发效率优先"约束，上调 1 个标度等级）：

```
开发效率:   0.38 ███████████████████
可扩展性:   0.28 ██████████████
可靠性:     0.20 ██████████
运维成本:   0.14 ███████
```

## Step 4: 一致性校验

- 开发效率 > 可扩展性 > 可靠性 > 运维成本 ✓
- 开发效率 > 可靠性 ✓（传递性满足）
- 可扩展性 > 运维成本 ✓（传递性满足）
- 通过一致性检查 ✓

## Step 5: 方案评分

| 子准则 | 权重 | 全面微服务 | 模块化单体 | 渐进式拆分 |
|-------|------|-----------|-----------|-----------|
| 水平扩展能力 | 0.16 | 9 | 5 | 7 |
| 独立部署能力 | 0.12 | 9 | 4 | 6 |
| 开发复杂度 | 0.14 | 4 | 8 | 7 |
| 团队协作效率 | 0.12 | 7 | 6 | 7 |
| 上线速度 | 0.12 | 4 | 9 | 7 |
| 故障隔离 | 0.12 | 9 | 4 | 6 |
| 数据一致性 | 0.08 | 5 | 9 | 7 |
| 基础设施成本 | 0.05 | 4 | 8 | 6 |
| 监控复杂度 | 0.05 | 3 | 8 | 6 |
| 运维人力需求 | 0.04 | 3 | 8 | 6 |

### 加权得分

| 方案 | 加权总分 |
|-----|---------|
| **渐进式拆分** | **6.62** |
| **模块化单体** | **6.52** |
| **全面微服务** | **6.35** |

### 敏感性分析

将"可扩展性"权重从 0.28 提升至 0.40（假设业务增长超预期）：
- 全面微服务: 6.62 → 第 1
- 渐进式拆分: 6.70 → 第 1（依然领先）
- 模块化单体: 5.98 → 第 3

渐进式拆分在两种场景下都保持领先，结论稳健。

## Step 6: 决策报告

# 决策报告：架构方案选型

## 推荐方案
**渐进式拆分** — 综合得分 6.62 / 10

## 排名

| 排名 | 方案 | 得分 | 核心优势 |
|-----|------|------|---------|
| 1 | 渐进式拆分 | 6.62 | 平衡短期效率与长期扩展性，风险最低 |
| 2 | 模块化单体 | 6.52 | 开发最快，但扩展性天花板明显 |
| 3 | 全面微服务 | 6.35 | 扩展性最强，但当前阶段复杂度过高 |

## 关键权衡
- 全面微服务在可扩展性和故障隔离上领先，但开发复杂度和运维成本过高
- 模块化单体短期效率最高，但当订单量突破 100K 时可能再次遇到瓶颈
- 渐进式拆分兼顾两者优势，但要求团队有清晰的模块边界设计能力

## 风险提示
- 渐进式拆分要求纪律性：若模块边界设计不清晰，后续拆分成本会急剧增加
- 需要提前识别最可能成为瓶颈的模块（订单处理、库存管理）作为首批拆分候选

## 建议
1. 第一阶段（1-2 月）：将现有 Django 单体严格模块化，建立清晰的模块边界
2. 第二阶段（3-4 月）：将订单处理模块拆分为独立服务
3. 第三阶段（按需）：根据实际瓶颈逐步拆分其他模块
4. 配套基建：引入 API Gateway、服务发现、分布式追踪
