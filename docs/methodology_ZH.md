...
**论文对应**：文档输入与语义树构建 —— Doc2AHP 中的 "Doc"

原始论文的核心创新是将**非结构化文档**通过 embedding 和层次聚类转化为结构化的 AHP 层次。Step 0 通过让用户显式选择信息来源，保留了这一关键的第一阶段。

**两种模式**：

| 维度 | 模式 A: 文档驱动 | 模式 B: 快速分析 |
|------|-----------------|----------------|
| 输入 | 用户提供的文档、URL 或网络搜索结果 | 用户的口头场景描述 |
| 准则提取 | 从文档内容中提取，带来源标注 | 从 LLM 领域知识中生成 |
| 论文契合度 | 忠实于 Doc2AHP 流水线 | 简化版 —— 跳过了 "Doc" 阶段 |
| 可追溯性 | 每个准则标注 `[来源: 文档名, 章节]` | 准则来源隐含在 LLM 知识中 |
| 适用场景 | 高风险决策、合规敏感、需要团队认同 | 成熟领域、快速探索、个人决策 |

**模式 A 的信息来源**：
1. **本地文件**：需求文档、技术规范、ADR、会议记录、基准测试报告
2. **网页 URL**：比较文章、官方文档、性能评测页面
3. **网络搜索**：Claude 主动搜索权威来源并综合分析
4. **混合模式**：以上任意组合

**模式 A: τ (tau) 相关性过滤**：
从所有文档提取候选准则后，执行相关性过滤：
- 保留出现在多个来源中、或与决策目标强相关的准则
- 过滤仅出现在单一来源且相关性较弱的准则
- 记录被过滤的准则及原因

**何时推荐模式 A 而非模式 B**：
- 决策影响多个利益相关方，需要解释决策依据
- 合规或审计要求可追溯的决策记录
- 决策领域高度专业（医疗、金融科技、安全），通用知识可能不足
- 团队已有现成的调研或分析文档值得利用

完整的模式 A 示例见 [文档驱动云服务商选型案例](../examples/doc-grounded-cloud-selection_ZH.md)。
...
2. **提取准则**：
   - **模式 A**：从 Step 0 处理的文档中提取，为每个准则标注来源引用
   - **模式 B**：从项目上下文、需求文档、团队讨论中提取评估维度
